...
```text
open / critical — сбор начат 2026-07-07 (ANCHOR_CASES/)
```

Заведена `ANCHOR_CASES/` с критерием приёма (внешний арбитр; арбитр не читал SynTax; предсказание сделано до проверки). Первый кейс принят как **слабый** (возможен скрытый recall из обучающих данных). Пробел остаётся открытым: один слабый кейс не валидирует runtime.
...
```text
theory drafted 2026-07-07 — see OPEN_GAPS_11b3_THEORY_DRAFT.md
DRAFT NOT VALIDATED: выведена под Расходящимся режимом, фазу отбора не проходила
```

Гипотеза: separation и completion конфликтуют **не по силе, а по времени**. Защита должна не выключаться на Design, а **откладываться** до фазы отбора. Следствие: v5.0 выключает M4 на Design и не возвращает — возможная механическая причина 11b.1.
